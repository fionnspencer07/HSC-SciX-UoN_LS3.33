# ============================================================================ #
# Comprehensive DGE and Visualization Pipeline for NK/T Cell Analysis (Run 2)
#
...
# (This function remains the same as it is flexible for any DGE result format)
create_dge_excel <- function(dge_results_list, method_name, output_dir) {
    ...
    addWorksheet(wb, "Trafficking_Genes_All_Contrasts"); writeData(wb, "Trafficking_Genes_All_Contrasts", all_contrasts_df %>% filter(gene %in% trafficking_genes))
    ...
    addWorksheet(wb, "Top100_Upregulated_by_Contrast"); writeData(wb, "Top100_Upregulated_by_Contrast", top_up)
    addWorksheet(wb, "Top100_Downregulated_by_Contrast"); writeData(wb, "Top100_Downregulated_by_Contrast", top_down)
...
# Create a SingleCellExperiment object from Seurat
sce <- as.SingleCellExperiment(seu_NKT_focused, assay = "RNA")
# Aggregate counts to sample level (summing across cells within each sample)
pb_counts <- aggregateAcrossCells(sce, 
                                  ids=colData(sce)[, "sample_name"], 
                                  use.assay.type="counts",
                                  average=FALSE) # We want sum of counts
# Get unique sample metadata
sample_metadata <- as.data.frame(colData(sce)) %>% distinct(sample_name, .keep_all = TRUE) %>% select(sample_name, treatment, Compartment) %>% column_to_rownames("sample_name")
# Ensure column data and counts matrix are in the same order
pb_counts <- pb_counts[, rownames(sample_metadata)]
# Create DESeqDataSet
dds <- DESeqDataSetFromMatrix(countData = assay(pb_counts),
...
Idents(seu_NKT_focused) <- "treatment"
mast_results <- list()
# Note: MAST can be slow. logfc.threshold = 0 ensures we get all genes.
...
# Helper function and color definitions
...
# 4.1 Volcano Plots (one for each method)
...
# 4.2 Heatmaps (using ComplexHeatmap)
# Heatmap 1: Average expression of gene sets (this is method-independent)
...
avg_exp_annot_df <- data.frame(col_names = colnames(avg_exp_grouped)) %>% separate(col_names, into = c("treatment", "Compartment"), sep = "_", remove = FALSE)
ha_avg <- HeatmapAnnotation(Treatment = avg_exp_annot_df$treatment, Compartment = avg_exp_annot_df$Compartment, col = list(Treatment = treatment_colors, Compartment = compartment_colors), annotation_name_side = "left")
...
# Heatmap 2: Top 50 DEGs from each method
plot_top50_heatmap <- function(dge_res, method_name) {
    ...
    ha_cells <- HeatmapAnnotation(Treatment = seu_NKT_focused$treatment, Compartment = seu_NKT_focused$Compartment, `Cell Type` = seu_NKT_focused$cell_types, col = list(Treatment = treatment_colors, Compartment = compartment_colors, `Cell Type` = cell_type_colors))
...
# 4.3 Other visualizations (Vln, Nebulosa, GSEA - these are method-independent or use Wilcoxon as representative)
...
save_svg(dittoPlot(seu_NKT_focused, "S1pr1", group.by = "treatment", split.by = "Compartment", plots = c("vlnplot", "jitter"), jitter.size = 0.1) + labs(title = "S1pr1 Expression by Treatment and Compartment"), "Vln_S1pr1_by_Compartment.svg", width = 12)
...
save_svg(plot_density(seu_NKT_focused, features = "S1pr1", reduction = "harmony", group.by = "Compartment") + labs(title = "S1pr1 Expression Density by Compartment (UMAP)"), "Nebulosa_S1pr1_by_Compartment.svg")

# GSEA using Wilcoxon ranks as it's fast and non-parametric
ranks <- wilcox_results[["UT_vs_NAIVE"]] %>% select(gene, avg_log2FC) %>% na.omit() %>% distinct(gene, .keep_all = TRUE) %>% deframe()
