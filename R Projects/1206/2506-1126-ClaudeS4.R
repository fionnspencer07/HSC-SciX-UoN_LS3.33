...
# Using ComplexHeatmap - silent version with minimal output
...
# Only show missing genes if there are any (and only the important ones)
...
# Run DEG analysis (suppress messages)
...
create_complexheatmap <- function(seu_obj, genes_list, title_suffix = "", 
  ...
  sample_info <- data.frame(
    condition = gsub("_(BM|WB)$", "", sample_names),
    compartment = ifelse(grepl("_BM$", sample_names), "BM", "WB"),
